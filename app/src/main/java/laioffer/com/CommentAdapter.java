...
public class CommentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void configureEventView(final EventViewHolder holder) {
        ...
        String[] location = event.getAddress().split(",");
        holder.eventLocation.setText(location[1] + "," + location[2]);
        ...
        if (event.getImgUri() != null) {
            ...
            new AsyncTask<Void, Void, Bitmap>(){
                ...
                protected Bitmap doInBackground(Void... voids) {
                    return Utils.getBitmapFromURL(uri);
                }
    ...
    private void configureCommentView(final CommentViewHolder commentViewHolder, final int position) {
        final Comment comment = commentList.get(position - 1);
        commentViewHolder.commentUser.setText(comment.getCommenter());
        commentViewHolder.commentDescription.setText(comment.getDescription());
        commentViewHolder.commentTime.setText(Utils.timeTransformer(comment.getTime()));
    }
}
